...
class WeightedAStar(SearchAlgorithm):
    def __init__(self, graph, weight=1.5):  # Default weight = 1.5
        """
        Initialize the Weighted A* search with a graph.

        :param graph: The SearchGraph object containing nodes and edges
        :param weight: The weight factor for the heuristic (w > 1)
        """
        super().__init__(graph)
        self.nodes = graph.node_coordinates
        self.weight = weight  # Store the weight factor
    ...
    def search(self, start, goals):
        """
        Execute Weighted A* Search from start node to any goal node.
        ...
        while open_list:
            ...
            for neighbor, cost in neighbors:
                ...
                if neighbor not in g_scores or new_g_score < g_scores[neighbor]:
                    ...
                    # Apply weighted heuristic
                    f_score = new_g_score + (self.weight * h_score)  # **Modified f(n)**
